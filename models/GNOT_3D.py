...
from utils.utilities3 import MultipleTensors
...
class Model(nn.Module):
    def __init__(self,
                 args,
                 trunk_size=3,
                 branch_sizes=None, # in_frames * space_dim
                 space_dim=2,
                 output_size=1,
                 n_layers=2,
                 n_hidden=128,
        ...
        self.trunk_size = args.in_dim * args.in_var + 1
        if branch_sizes is None:
            branch_sizes = [args.in_dim * args.in_var, ]
        ...
        self.output_size = args.out_dim * args.out_var
        # self.space_dim = space_dim
        self.space_dim = args.in_dim * args.in_var
        n_hidden = args.d_model
        ...
        self.gpt_config = MoEGPTConfig(attn_type=attn_type,embd_pdrop=ffn_dropout, resid_pdrop=ffn_dropout, attn_pdrop=attn_dropout,n_embd=n_hidden, n_head=n_head, n_layer=n_layers,
                                       block_size=128,act=act, n_experts=n_experts,space_dim=self.space_dim, branch_sizes=branch_sizes,n_inputs=0,n_inner=n_inner)
        ...
        self.out_mlp = MLP(n_hidden, n_hidden, self.output_size, n_layers=mlp_layers)
    ...
    def forward(self, x, fx=None, inputs=None, T=None):
        b, _, h, w, c = x.shape
        x = rearrange(x, 'b z h w c -> b (z h w) c') # b n t*v
        pos = x[:,:,0:self.space_dim] # b n t*v
        u_p = torch.zeros([1, 1]).cuda()
        inputs = [pos, ] # b n t*v
        B, N, C = x.shape
        x = torch.cat([x, u_p.unsqueeze(1).repeat([B, N, 1])], dim=-1) # b n t*v+1

        # if self.horiz_fourier_dim > 0:
        #     x = horizontal_fourier_embedding(x, self.horiz_fourier_dim)
        #     z = horizontal_fourier_embedding(z, self.horiz_fourier_dim)

        x = self.trunk_mlp(x) # b n c
        z = MultipleTensors([self.branch_mlps[i](inputs[i]) for i in range(self.n_inputs)])

        for block in self.blocks:
            x = block(x, z, pos)
        x = self.out_mlp(x)
        x = rearrange(x, 'b (z h w) c -> b z h w c', h=h, w=w)
        # x_out = torch.cat([x[i, :num] for i, num in enumerate(g.batch_num_nodes())],dim=0)
        return x