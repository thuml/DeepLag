...
from .libs.fact.factorization_module import FABlock3D
...
class FactorizedTransformer(nn.Module):
    def __init__(self,
        ...
        for _ in range(depth):
            ...
            layer.append(FABlock3D(dim, dim_head, dim, heads, dim_out, use_rope=True,
                                   **kwargs))
            self.layers.append(layer)

    def forward(self, u, pos_lst=None):
        b, nz, nx, ny, c = u.shape
        # nz, nx, ny = pos_lst[0].shape[0], pos_lst[1].shape[0], pos_lst[2].shape[0]
        ...
        for pos_enc, attn_layer in self.layers:
            tmp = pos_enc(pos).view(1, nz, nx, ny, -1)
            u += tmp
    ...
    def get_grid(self, shape, device):
        ...
        gridz = gridz.reshape(1, size_z, 1, 1, 1)
        gridx = torch.tensor(np.linspace(0, 1, size_x), dtype=torch.float)
        gridx = gridx.reshape(1, 1, size_x, 1, 1)
        gridy = torch.tensor(np.linspace(0, 1, size_y), dtype=torch.float)
        gridy = gridy.reshape(1, 1, 1, size_y, 1)
        return torch.cat((gridz.repeat([1, 1, size_x, size_y, 1]), gridx.repeat([1, size_z, 1, size_y, 1]), gridy.repeat([1, size_z, size_x, 1, 1])), dim=-1).to(device), \
            [gridz[0,:,:,0,0], gridx[0,0,:,:,0], gridy[0,0,0,:,:]]
...
class Model(nn.Module):
    ...
    def forward(self,
        ...
        pos_lst = None
        # b, _, c = u.shape
        # u = u.view(b, self.Z, self.H, self.W, c)
        _, nz, nx, ny, _ = u.shape
        u = self.to_in(u)
        u_last = self.encoder(u, pos_lst)
        u = rearrange(u_last, 'b nz nx ny c -> b c nz nx ny')
        assert u.shape[1] == self.dim
        u = self.down_block(u)
        u = self.up_block(u)
        u = rearrange(u, 'b c nz nx ny -> b nz nx ny c')
        # print(u.shape, u_last.shape)
        u = torch.cat([u, u_last], dim=-1)
        u = self.simple_to_out(u)
        u = rearrange(u, 'b c (nz nx ny) -> b nz nx ny c', nx=nx, ny=ny)
        return u