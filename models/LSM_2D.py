...
class NeuralSpectralBlock2d(nn.Module):
    ...
    def latent_encoder_attn(self, x):
        ...
        B, C, H, W = x.shape
        L = H * W
        latent_token = self.latent[None, :, :, :].repeat(B, 1, 1, 1) # B, head, num_token, head_channel
        
        x_tmp = self.encoder_attn(x).view(B, C * 2, -1).permute(0, 2, 1).contiguous() # B, patch_size, C * 2
        x_tmp = x_tmp.view(B, L, self.head, C // self.head, 2).permute(4, 0, 2, 1, 3).contiguous() # 2, B, head, patch_size, head_channel
    ...
    def forward(self, x):
        ...
        # patchify
        # B, C, H, W -> B,
        x = x.view(B, C, H // self.patch_size[0], self.patch_size[0], W // self.patch_size[1], self.patch_size[1]).contiguous() \
            .permute(0, 2, 4, 1, 3, 5).contiguous() \
            .view(B * (H // self.patch_size[0]) * (W // self.patch_size[1]), C,
                  self.patch_size[0],
                  self.patch_size[1])
