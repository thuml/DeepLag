...
class Model(nn.Module):
    def __init__(self, args):
        ...
        self.convs = nn.ModuleList([SpectralConv2d(self.width, self.width, self.modes1, self.modes2) for _ in range(args.num_layers)])
        # self.w0 = nn.Conv2d(self.width, self.width, 1)
        # self.w1 = nn.Conv2d(self.width, self.width, 1)
        # self.w2 = nn.Conv2d(self.width, self.width, 1)
        # self.w3 = nn.Conv2d(self.width, self.width, 1)
        self.ws = nn.ModuleList([nn.Conv2d(self.width, self.width, 1) for _ in range(args.num_layers)])
...
if __name__ == "__main__":
    conv = SpectralConv2d(in_channels=64, out_channels=64, modes1=32, modes2=32)
