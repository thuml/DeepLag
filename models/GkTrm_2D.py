...
class DownScaler(nn.Module):
    ...
    def forward(self, x):
        ...
        '''
        n_grid_h, n_grid_w = x.size(1), x.size(2)
        bsz = x.size(0)
        x = x.view(bsz, n_grid_h, n_grid_w, self.in_dim)
        x = x.permute(0, 3, 1, 2)
        x = self.downsample(x)
        x = x.permute(0, 2, 3, 1)
        return x
...
class Model(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.downscaler_size, self.upscaler_size = self._get_scaler_sizes((self.fine_h, self.fine_w), (self.attn_h, self.attn_w))
        pos, elem = self._get_grid((self.attn_h, self.attn_w))
        pos_fine = self._get_grid((self.fine_h, self.fine_w),
    ...
    def forward(self, node, edge=None, pos=None, grid=None, weight=None, boundary_value=None):
        ...
        '''
        bsz = node.size(0)
        n_h, n_w = self.fine_h, self.fine_w
        n_s_h, n_s_w = self.attn_h, self.attn_w
        if pos is None:
            pos = self.pos.to(node.device).view(1, n_s_h*n_s_w, 2).repeat(bsz, 1, 1)
        if grid is None:
            grid = self.grid.to(node.device).view(1, n_h, n_w, 2).repeat(bsz, 1, 1, 1)
        ...
        if not self.downscaler_size:
            node = torch.cat(
                [node, pos.contiguous().view(bsz, n_s_h, n_s_w, -1)], dim=-1)
        ...
        x = x.view(bsz, n_s_h, n_s_w, self.n_hidden)
    ...
    def _get_grid(n_grid, subsample=1, return_elem=True, return_boundary=True):
        x = np.linspace(0, 1, n_grid[0])
        y = np.linspace(0, 1, n_grid[1])
        x, y = np.meshgrid(x, y)
    ...
    def _get_scaler_sizes(n_f, n_c, scale_factor=True):
        ...
        n_f_h, n_f_w = n_f
        n_c_h, n_c_w = n_c
        factor_h, factor_w = get_factor(n_f_h, n_c_h), get_factor(n_f_w, n_c_w)
        down_factor = (factor_h, factor_w)
        n_m_h, n_m_w = round(n_f_h*factor_h)-1, round(n_f_w*factor_w)-1
        down_size = ((n_m_h, n_m_w), (n_c_h, n_c_w))
        up_size = ((n_m_h, n_m_w), (n_f_h, n_f_w))
        if scale_factor:
