...
from .basics import PreNorm, PostNorm, GeAct, MLP, masked_instance_norm
...
class FABlock2D(nn.Module):
    ...
    def __init__(self,
        ...
        self.instance = nn.InstanceNorm2d(dim_head * heads)
    ...
    def forward(self, u, pos_lst):
        ...
        u_phi = u_phi.permute(0, 3, 1, 2).contiguous()
        u_phi = self.instance(u_phi)
        u_phi = u_phi.permute(0, 2, 3, 1).contiguous()
        ret = self.to_out(u_phi)
        return ret
