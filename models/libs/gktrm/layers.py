...
class Shortcut3d(nn.Module):
    '''
    (-1, in, S, S, S) -> (-1, out, S, S, S)
    Used in SimpleResBlock
    '''

    def __init__(self, in_features=None,
                 out_features=None,):
        super(Shortcut3d, self).__init__()
        self.shortcut = nn.Linear(in_features, out_features)
...
class Conv3dResBlock(nn.Module):
    ...
    def __init__(self, in_dim, out_dim,
        ...
        if self.add_res:
            if self.apply_shortcut:
                self.res = Shortcut3d(in_dim, out_dim)
            else:
                self.res = Identity()
...
class Conv3dEncoder(nn.Module):
    ...
    def __init__(self, in_dim: int,
        ...
        padding2 = padding//4 if padding//4 >= 1 else 1
        activation_type = default(activation_type, 'silu')
        self.conv0 = Conv3dResBlock(in_dim, out_dim, kernel_size=kernel_size,
                                    padding=padding,
                                    residual=residual)
        self.conv1 = Conv3dResBlock(out_dim, conv_dim0, kernel_size=kernel_size,
                                    padding=padding1,
                                    stride=stride, residual=residual)
        self.conv2 = Conv3dResBlock(conv_dim0, conv_dim1, kernel_size=kernel_size,
                                    dilation=dilation,
                                    padding=padding2, residual=residual)
        self.conv3 = Conv3dResBlock(conv_dim1, conv_dim2,
                                    kernel_size=kernel_size,
                                    residual=residual)
        self.pool0 = nn.AvgPool3d(kernel_size=scaling_factor,
                                  stride=scaling_factor)
        self.pool1 = nn.AvgPool3d(kernel_size=scaling_factor,
                                  stride=scaling_factor)
        self.activation = nn.SiLU() if activation_type == 'silu' else nn.ReLU()
        # self.activation = nn.LeakyReLU() # leakyrelu decreased performance 10 times?
        self.debug = debug
...
class Interp3dEncoder(nn.Module):
    r'''
    Using Interpolate instead of avg pool
    interp dim hard coded or using a factor
    old code uses lambda and cannot be pickled
    ...
    def __init__(self, in_dim: int,
                 ...
                 dropout=0.1,
                 debug=False):
        super(Interp3dEncoder, self).__init__()
        ...
        self.interp_size = interp_size
        self.is_scale_factor = isinstance(
            interp_size[0], float) and isinstance(interp_size[1], float) and isinstance(interp_size[2], float)
        self.conv0 = Conv3dResBlock(in_dim, out_dim, kernel_size=kernel_size,
                                    padding=padding, activation_type=activation_type,
                                    dropout=dropout,
                                    residual=residual)
...
class DeConv3dBlock(nn.Module):
    ...
    def __init__(self, in_dim: int,
        ...
        self.deconv0 = nn.ConvTranspose3d(in_channels=in_dim,
                                          ...
                                          output_padding=output_padding,
                                          padding=padding)
        self.deconv1 = nn.ConvTranspose3d(in_channels=hidden_dim,
                                          out_channels=out_dim,
                                          kernel_size=kernel_size,
                                          stride=stride,
                                          output_padding=output_padding,
...
class SpectralConv2d(nn.Module):
    ...
    def forward(self, x):
        ...
        if n_dim == 4:
            ...
            # n = int(x.size(1)**(0.5))
            raise Exception('height and width cannot be put into a single dimension')
        else:
        ...
        x = x.view(-1, nh, nw, in_dim)
        res = self.linear(x)
        x = self.dropout(x)

        x = x.permute(0, 3, 1, 2)
        x_ft = fft.rfft2(x, s=(nh, nw), norm=self.norm)
        x_ft = torch.stack([x_ft.real, x_ft.imag], dim=-1)

        out_ft = torch.zeros(batch_size, out_dim, nh, nw //
        ...
        x = fft.irfft2(out_ft, s=(nh, nw), norm=self.norm)
        x = x.permute(0, 2, 3, 1)
        x = self.activation(x + res)

        if n_dim == 3:
            x = x.view(batch_size, nh*nw, out_dim)
...
class SpectralConv3d(nn.Module):
    def __init__(self, in_dim,
        ...
        '''
        Modified Zongyi Li's SpectralConv3d PyTorch 1.6 code
        using only real weights
        https://github.com/zongyi-li/fourier_neural_operator/blob/master/fourier_2d.py
        '''
        self.in_dim = in_dim
        ...
        self.fourier_weight = nn.ParameterList([Parameter(
            torch.FloatTensor(in_dim, out_dim,
                                                modes, modes, modes, 2)) for _ in range(4)])
        for param in self.fourier_weight:
            xavier_normal_(param, gain=1/(in_dim*out_dim)
                           * np.sqrt(in_dim+out_dim))
        self.dropout = nn.Dropout(dropout)
    ...
    def forward(self, x):
        ...
        n_dim = x.ndim
        if n_dim == 5:
            nz, nh, nw = x.size(1), x.size(2), x.size(3)
            # assert x.size(1) == x.size(2) and x.size(2) == x.size(3)
        elif n_dim == 3:
            # n = int(x.size(1)**(1/3))
            raise Exception('layer, height and width cannot be put into a single dimension')
        else:
            raise ValueError("Dimension not implemented")
        in_dim = self.in_dim
        out_dim = self.out_dim
        ...
        x = x.view(-1, nz, nh, nw, in_dim)
        res = self.linear(x)
        x = self.dropout(x)

        x = x.permute(0, 4, 1, 2, 3)
        x_ft = fft.rfftn(x, s=(nz, nh, nw), norm=self.norm)
        x_ft = torch.stack([x_ft.real, x_ft.imag], dim=-1)

        out_ft = torch.zeros(batch_size, out_dim, nz, nh, nw //
                             2+1, 2, device=x.device)
        out_ft[:, :, :modes, :modes, :modes] = self.complex_matmul_3d(
            x_ft[:, :, :modes, :modes, :modes], self.fourier_weight[0])
        out_ft[:, :, -modes:, :modes, :modes] = self.complex_matmul_3d(
            x_ft[:, :, -modes:, :modes, :modes], self.fourier_weight[1])
        out_ft[:, :, :modes, -modes:, :modes] = self.complex_matmul_3d(
            x_ft[:, :, :modes, -modes:, :modes], self.fourier_weight[2])
        out_ft[:, :, -modes:, -modes:, :modes] = self.complex_matmul_3d(
            x_ft[:, :, -modes:, -modes:, :modes], self.fourier_weight[3])
        out_ft = torch.complex(out_ft[..., 0], out_ft[..., 1])

        x = fft.irfftn(out_ft, s=(nz, nh, nw), norm=self.norm)
        x = x.permute(0, 2, 3, 4, 1)
        x = self.activation(x + res)

        if n_dim == 3:
            x = x.view(batch_size, nz*nh*nw, out_dim)
