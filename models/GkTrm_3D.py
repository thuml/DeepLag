from .libs.gktrm.layers import *
# from .libs.gktrm.utils_ft import *
...
class DownScaler(nn.Module):
    def __init__(self, in_dim,
        ...
        '''
        A wrapper for conv3d/interp downscaler
        '''
        if downsample_mode == 'conv':
            self.downsample = nn.Sequential(Conv3dEncoder(in_dim=in_dim,
                                                          out_dim=out_dim,
                                                          activation_type=activation_type,
                                                          debug=debug),
                                            Conv3dEncoder(in_dim=out_dim,
                                                          out_dim=out_dim,
                                                          padding=padding,
                                                          activation_type=activation_type,
                                                          debug=debug))
        elif downsample_mode == 'interp':
            self.downsample = Interp3dEncoder(in_dim=in_dim,
                                              out_dim=out_dim,
    ...
    def forward(self, x):
        '''
        3D:
            Input: (-1, n, n, n, n, in_dim)
            Output: (-1, n_s, n_s, n_s, n_s, out_dim)
        '''
        n_grid_z, n_grid_h, n_grid_w = x.size(1), x.size(2), x.size(3)
        bsz = x.size(0)
        x = x.view(bsz, n_grid_z, n_grid_h, n_grid_w, self.in_dim)
...
class UpScaler(nn.Module):
    def __init__(self, in_dim: int,
        ...
        if upsample_mode in ['conv', 'deconv']:
            ...
            self.upsample = Interp3dUpsample(in_dim=in_dim,
                                             out_dim=out_dim,
    ...
    def forward(self, x):
        '''
        2D:
            Input: (-1, n_s, n_s, n_s, in_dim)
            Output: (-1, n, n, n, out_dim)
        '''
        x = x.permute(0, 4, 1, 2, 3)
        x = self.upsample(x)
        x = x.permute(0, 2, 3, 4, 1)
...
class Model(nn.Module):
    def __init__(self, **kwargs):
        ...
        self.config = defaultdict(lambda: None, **kwargs)['args']

        # downscaler
        self.node_feats = self.config.in_dim * self.config.in_var
        self.spacial_dim = 3
        self.n_hidden = self.config.d_model
        self.downsample_mode = 'interp'
        self.subsample_nodes = 1
        self.subsample_attn = 4
        self.downscaler_dropout = 0.05
        self.downscaler_activation = 'relu'
        # upscaler
        self.upsample_mode = 'interp'
        self.upscaler_dropout = 0.0
        self.upscaler_activation = 'silu'

        # feature extractor
        self.feat_extract_type = None
        self.edge_feats = 0
        self.num_feat_layers = 0
        self.graph_activation = None
        self.raw_laplacian = None

        # encoder
        self.num_encoder_layers = 6
        self.attention_type = 'galerkin'
        self.n_head = 4
        ...
        self.pos_dim = 3
        self.xavier_init = 0.01
        self.diagonal_weight = 0.01
        self.symmetric_init = False
        self.return_attn_weight = False
        self.encoder_dropout = 0.05
        self.ffn_dropout = 0.05
        self.norm_eps = 0.0000001

        # regressor (decoder)
        self.decoder_type = 'ifft2'
        self.freq_dim = 32
        self.n_targets = self.config.out_dim * self.config.out_var
        self.num_regressor_layers = 2
        self.fourier_modes = 12
        self.spacial_fc = True
        self.regressor_activation = 'silu'
        self.last_activation = True
        ...
        self.return_boundary = True
        self.img_z = int(((self.config.z - 1) / self.config.z_down) + 1)
        self.img_h = int(((self.config.h - 1) / self.config.h_down) + 1)
        self.img_w = int(((self.config.w - 1) / self.config.w_down) + 1)
        self.fine_z = int(((self.img_z - 1) / self.subsample_nodes) + 1)
        self.fine_h = int(((self.img_h - 1) / self.subsample_nodes) + 1)
        self.fine_w = int(((self.img_w - 1) / self.subsample_nodes) + 1)
        self.attn_z = int(((self.img_z - 1) / self.subsample_attn) + 1)
        self.attn_h = int(((self.img_h - 1) / self.subsample_attn) + 1)
        self.attn_w = int(((self.img_w - 1) / self.subsample_attn) + 1)
        self.downscaler_size, self.upscaler_size = self._get_scaler_sizes((self.fine_z, self.fine_h, self.fine_w), (self.attn_z, self.attn_h, self.attn_w))
        pos, elem = self._get_grid((self.attn_z, self.attn_h, self.attn_w))
        pos_fine = self._get_grid((self.fine_z, self.fine_h, self.fine_w),
                                  subsample=self.subsample_nodes, # 1
                                  return_elem=False,
                                  return_boundary=self.return_boundary) # true
        self.pos = torch.from_numpy(pos).float()
        self.grid = torch.from_numpy(pos_fine).float()
    ...
    def forward(self, node, edge=None, pos=None, grid=None, weight=None, boundary_value=None):
        '''
        - node: (batch_size, n, n, n, node_feats)
        - pos: (batch_size, n_s*n_s*n_s, pos_dim)
        - edge: (batch_size, n_s*n_s*n_s, n_s*n_s*n_s, edge_feats)
        - weight: (batch_size, n_s*n_s*n_s, n_s*n_s*n_s): mass matrix prefered
            or (batch_size, n_s*n_s*n_s) when mass matrices are not provided (lumped mass)
        - grid: (batch_size, n-2, n-2, n-2, 3) excluding boundary
        '''
        bsz = node.size(0)
        n_z, n_h, n_w = self.fine_z, self.fine_h, self.fine_w
        n_s_z, n_s_h, n_s_w = self.attn_z, self.attn_h, self.attn_w
        if pos is None:
            pos = self.pos.to(node.device).view(1, n_s_z*n_s_h*n_s_w, 3).repeat(bsz, 1, 1)
        if grid is None:
            grid = self.grid.to(node.device).view(1, n_z, n_h, n_w, 3).repeat(bsz, 1, 1, 1, 1)
        ...
        if not self.downscaler_size:
            node = torch.cat(
                [node, pos.contiguous().view(bsz, n_s_z, n_s_h, n_s_w, -1)], dim=-1)
        x = self.downscaler(node)
        x = x.view(bsz, -1, self.n_hidden)

        if self.num_feat_layers > 0 or edge is not None:
        ...
        x = x.view(bsz, n_s_z, n_s_h, n_s_w, self.n_hidden)
        x = self.upscaler(x) # b n n n_hidden
        ...
        if self.boundary_condition == 'dirichlet':
            x = x[:, 1:-1, 1:-1, 1:-1].contiguous()
            x = F.pad(x, (0, 0, 1, 1, 1, 1, 1, 1), "constant", 0)
            if boundary_value is not None:
                assert x.size() == boundary_value.size()
                x += boundary_value

        if self.return_latent or self.return_attn_weight:
            return dict(preds=x,
                    preds_latent=x_latent,
                    attn_weights=attn_weights)
        else:
    ...
    def _get_setting(self):
        all_attr = list(self.config.__dict__.keys()) # + ADDITIONAL_ATTR
        for key in all_attr:
    ...
    @staticmethod
    def _get_grid(n_grid, subsample=1, return_elem=True, return_boundary=True):
        z = np.linspace(0, 1, n_grid[0])
        x = np.linspace(0, 1, n_grid[1])
        y = np.linspace(0, 1, n_grid[2])
        z, x, y = np.meshgrid(z, x, y)
        nz, nx, ny = n_grid  # uniform grid
        s = subsample

        if return_elem:
            grid = np.c_[z.ravel(), x.ravel(), y.ravel()]  # (n_node, 3)
            elem = []
            # for j in range(ny-1):
            #     for i in range(nx-1):
            #         a = i + j*nx
            ...
            if return_boundary:
                ...
                z = z[::s, ::s, ::s][1:-1, 1:-1, 1:-1]
                x = x[::s, ::s, ::s][1:-1, 1:-1, 1:-1]
                y = y[::s, ::s, ::s][1:-1, 1:-1, 1:-1]
            grid = np.stack([z, x, y], axis=-1)
            return grid
    
    @staticmethod
    def _get_scaler_sizes(n_f, n_c, scale_factor=True):
        def get_factor(l_f, l_c):
            factor = np.sqrt(l_c/l_f)
        ...
        n_f_z, n_f_h, n_f_w = n_f
        n_c_z, n_c_h, n_c_w = n_c
        factor_z, factor_h, factor_w = get_factor(n_f_z, n_c_z), get_factor(n_f_h, n_c_h), get_factor(n_f_w, n_c_w)
        down_factor = (factor_z, factor_h, factor_w)
        n_m_z, n_m_h, n_m_w = round(n_f_z*factor_z)-1, round(n_f_h*factor_h)-1, round(n_f_w*factor_w)-1
        down_size = ((n_m_z, n_m_h, n_m_w), (n_c_z, n_c_h, n_c_w))
        up_size = ((n_m_z, n_m_h, n_m_w), (n_f_z, n_f_h, n_f_w))
        if scale_factor:
